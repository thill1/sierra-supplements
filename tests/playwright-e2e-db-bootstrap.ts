/**
 * Ensures Docker e2e Postgres (if needed), schema, admin seed, and `.playwright/e2e-runtime.env`.
 * Must run before `next start` in the webServer process — Playwright starts webServer before globalSetup.
 */
...
export async function bootstrapPlaywrightE2eDatabase(): Promise<void> {
    ...
    if (!databaseUrl) {
        if (await tryStartDockerPostgres(root)) {
            ...
            throw new Error(
                "[playwright e2e DB] No DATABASE_URL and Docker Postgres could not start. " +
