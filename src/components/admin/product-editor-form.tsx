    ...
    work = [
      {
        ...
        stockQuantity:
          v.stockQuantity.trim() !== "" ? v.stockQuantity : form.stockQuantity,
        lowStockThreshold:
          v.lowStockThreshold.trim() !== ""
            ? v.lowStockThreshold
            : form.lowStockThreshold,
        stripePriceId:
          v.stripePriceId.trim() !== "" ? v.stripePriceId : form.stripePriceId,
      },
    ];
  }
  ...
  return {
    variants: work.map((v, idx) => ({
      ...
      stockQuantity: Number.isFinite(parseInt(v.stockQuantity, 10))
        ...
        : 0,
      lowStockThreshold: Number.isFinite(parseInt(v.lowStockThreshold, 10))
...
export function ProductEditorForm({
  ...
  const canManageCatalog = useCanManageCatalog();
          ...
          : [
              {
                ...
                lowStockThreshold: initial?.lowStockThreshold ?? "2",
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (isEdit) {
        ...
        const variantPayload = variantRowsToApiPayload(variants, form, false);
        ...
        if (!resVar.ok) {
          if (resVar.status === 403) {
            ...
            throw new Error(await getAdminApiErrorMessage(resVar));
          }
        } else {
      ...
      <div className="max-w-xl space-y-6">
        ...
        <div className="card p-6 space-y-3">
          ...
          <p className="text-sm text-[var(--color-text-secondary)]">
            Only managers and owners can create catalog products. Ask an owner
            to adjust your role, or have a manager add the product.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl">
      ...
      <form onSubmit={submit} className="space-y-8">
        ...
        <section
          className="card space-y-4 p-6 scroll-mt-24 ring-2 ring-[var(--color-accent)]/25"
          id="product-flavor-size-variants"
        >
          {!canManageCatalog && (
            <p className="text-xs rounded-md bg-amber-500/10 border border-amber-500/30 px-3 py-2 text-amber-950 dark:text-amber-100">
              Variant pricing, stock, and Stripe IDs can only be changed by a
              manager or owner. Product details above still save.
            </p>
          )}
          ...
          <p className="text-xs text-[var(--color-text-muted)]">
            Add one row per flavor, size, or other option. Customers choose on
            the store product page (radio or dropdown). Each variant can have
            its own price, SKU, Stripe price ID, and stock. For a single SKU,
            keep one row (label it clearly, e.g. &quot;2 lb&quot;); you can use
            the Pricing and Inventory fields below and leave duplicate fields
            here blank — they copy on save.
          </p>
          <fieldset
            disabled={!canManageCatalog}
            className="space-y-4 border-0 min-w-0 p-0 m-0 disabled:opacity-65"
          >
        ...
        <section className="card space-y-4 p-6">
          ...
          {multiVariant && (
            <p className="text-sm text-[var(--color-text-muted)]">
              This product has multiple variants. Stock and low-stock alerts are
              set per variant in Flavor & size above; product totals stay in
              sync automatically.
            </p>
          )}
          <div className="grid grid-cols-2 gap-4">
          ...
          <p className="text-xs text-[var(--color-text-muted)]">
            Prefer adjusting counts from the Inventory page so every change is
            logged.
          </p>
        </section>
        ...
        <section className="card space-y-4 p-6">
          ...
          {!isEdit && (
            <div className="space-y-2">
              <p className="text-sm text-[var(--color-text-muted)]">
                Extra photos (optional) — attached after the product is created.
              </p>
              <StagingUploader
                urls={stagingUrls}
                onAdd={(u) => setStagingUrls((s) => [...s, u])}
              />
            </div>
          )}
